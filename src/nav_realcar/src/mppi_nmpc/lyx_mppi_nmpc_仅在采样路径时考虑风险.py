...
class MPPI_CuPy:
    ...
    def _setup_nmpc(self) -> None:
        ...
        P = ca.SX.sym('P', n_states + n_states*(self.N+1) + n_controls*self.N + 2*self.N)
        ...
        for i in range(self.N):
            ...
            # 可通行性代价项 -(W_μ·μ + W_ν·ν)
            # 注：负号是因为我们要最大化可通行性
            trav_cost = -(self.cfg.W_mu * trav_mu[i] + self.cfg.W_nu * trav_nu[i]) * self.cfg.dt
            cost += trav_cost
    ...
    def _solve_nmpc(self, state0, ref_states, ref_controls):
        ...
        try:
            ...
            p = np.zeros(n_states + n_states*(N+1) + n_controls*N + 2*N)
