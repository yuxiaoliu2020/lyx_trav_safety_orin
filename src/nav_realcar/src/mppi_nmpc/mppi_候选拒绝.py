...
class MPPI_CuPy:
    ...
    def _build_rollout_kernel(self) -> None:
        ...
        void rollout_kernel(
            ...
            const float risk_threshold)          // 新增风险阈值参数
        {
            ...
            bool trajectory_rejected = false;  // 轨迹是否被拒绝的标志
            ...
            for (int t = 0; t < N; ++t)
            {
                ...
                // 候选拒绝（Hard-mask）策略：检查风险是否超过阈值
                if (risk_val > risk_threshold) {
                    // 轨迹被拒绝，设置极高的成本值
                    cost = 1e10f;  // 足够大的值，确保该轨迹不会被选中
    ...
    def _rollout_kernel_launch(self, state0: cp.ndarray, goal: cp.ndarray) -> None:
        ...
        risk_threshold = _as_f32(self.cfg.risk_threshold)  # 获取风险阈值
