...
# from .temporal_fusion_locattn_v4 import TemporalModel
...
class TravNet(nn.Module):
    ...
    def __init__(
        ...
        self.features_refine_safety = nn.Sequential(
            nn.Conv2d(self.latent_dim, self.latent_dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(self.latent_dim),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.latent_dim, self.latent_dim, kernel_size=3, padding=2, dilation=2, bias=False),
            nn.BatchNorm2d(self.latent_dim),
            nn.ReLU(inplace=True),
            nn.Dropout2d(0.1)
        )
        ...
        # 输出单通道安全系数图，范围 [0, 1]
        self.safetymap_head = nn.Sequential(
            nn.Conv2d(self.latent_dim, self.latent_dim, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(self.latent_dim),
            nn.ReLU(inplace=True),
            nn.Conv2d(self.latent_dim, 1, kernel_size=1, padding=0),
            nn.Sigmoid())
    ...
    def forward(self, color_img, pcloud, intrinsics, extrinsics, depth_img=None):
        ...
        bev_features_trav = self.features_refine(bev_features)
        trav_map = self.travmap_head(bev_features_trav)

        bev_features_safety = self.features_refine_safety(bev_features)
        safety_map = self.safetymap_head(bev_features_safety)
