...
class TraversabilityInferenceNode:
    ...
    def timer_callback(self, event):
        ...
        try:
            ...
            # 获取时间戳
            sensor_stamp = rgb_msg.header.stamp
            sensor_stamp_float = sensor_stamp.to_sec()
            # 小车上zed2i的相机内参
            intrinsics = np.array([
                [267.158, 0.0, 311.137],
                [0.0, 267.158, 176.519],
            ...
            # 更新当前帧
            self.update_current_frame(color_img, depth_img, sensor_stamp_float, intrinsics, cam2base, position, quaternion)
            # 检查是否有足够的历史帧来进行推理
            if self.frame_buffer.is_full():
                # 准备推理数据
                inference_data = self.prepare_inference_data()
                inference_data['sensor_stamp'] = sensor_stamp
                inference_data['sensor_stamp_float'] = sensor_stamp_float
    ...
    def inference_loop(self):
        ...
        with torch.no_grad():
            while not rospy.is_shutdown():
                ...
                sensor_stamp_float = data.get('sensor_stamp_float', None)
                ...
                result_data = {
                    ...
                    'sensor_stamp_float': sensor_stamp_float
                }
    ...
    def publishing_loop(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                sensor_stamp_float = result.get('sensor_stamp_float', None)
                ...
                pub_stamp = sensor_stamp if sensor_stamp is not None else ros_timestamp
                # 使用校正后的数据
                trav_map_msg = self.create_traversability_map_msg(corrected_trav_map_np, pub_stamp)
                self.trav_map_pub.publish(trav_map_msg)
                safety_map_msg = self.create_safety_map_msg(corrected_safety_map_np, pub_stamp)
                self.safety_map_pub.publish(safety_map_msg)
                
                mu_grid_msg = self.create_occupancy_grid_msg(corrected_trav_map_np[0, 0], pub_stamp, "mu")
                nu_grid_msg = self.create_occupancy_grid_msg(corrected_trav_map_np[0, 1], pub_stamp, "nu")
                safety_grid_msg = self.create_occupancy_grid_msg(corrected_safety_map_np[0, 0], pub_stamp, "safety")
                ...
                self.publish_image_messages(corrected_trav_map_np, corrected_safety_map_np, pub_stamp)
