...
from trav_est_realcar.msg import TraversabilityMap
...
class TraversabilityInferenceNode:
    def __init__(self):
        # 初始化ROS节点
        rospy.init_node('traversability_inference_node')
        ...
        self.model_path = rospy.get_param('~model_path', '/home/lyx/lyx_trav_risk_nav/src/trav_est_realcar/src/checkpoints/wayfaster.ckpt')
        ...
        # 添加2Hz定时器
        self.timer = rospy.Timer(rospy.Duration(0.2), self.timer_callback)  # 2Hz = 0.5秒
        ...
        rospy.loginfo("通过性推理节点初始化完成！设置为2Hz处理频率")
        ...
        try:
            ...
            if self.frame_buffer.is_full():
                ...
                if not self.inference_queue.full():
                    ...
                    rospy.loginfo_throttle(2.0, f"以2Hz频率处理帧，预处理耗时: {preprocess_time:.3f}秒")
                else:
                    rospy.logwarn("推理队列已满，跳过当前帧")
    ...
    def prepare_inference_data(self):
        ...
        for t in range(self.n_frames):
            ...
            # 尺寸调整
            if (self.config.MODEL.INPUT_SIZE[0] != color_img.shape[1]) or (self.config.MODEL.INPUT_SIZE[1] != color_img.shape[0]):
                intrinsics_list[t][0, :] *= (self.config.MODEL.INPUT_SIZE[0]/color_img.shape[1])
                intrinsics_list[t][1, :] *= (self.config.MODEL.INPUT_SIZE[1]/color_img.shape[0])
                color_img = cv2.resize(color_img, self.config.MODEL.INPUT_SIZE, interpolation=cv2.INTER_AREA)
                depth_img = cv2.resize(depth_img, self.config.MODEL.INPUT_SIZE, interpolation=cv2.INTER_NEAREST)
            
            # 转换为tensor格式
            color_img_list[t] = torch.from_numpy(color_img).permute(2,0,1).type(torch.float32) / 255.0
            
            # 深度图处理
            depth_target = depth_img
            depth_target[~np.isfinite(depth_target)] = 0
            depth_size = (int(np.ceil(self.config.MODEL.INPUT_SIZE[0]/self.config.MODEL.DOWNSAMPLE)), 
                        int(np.ceil(self.config.MODEL.INPUT_SIZE[1]/self.config.MODEL.DOWNSAMPLE)))
        ...
        pcloud_data = torch.from_numpy(pcloud_data).unsqueeze(0).type(torch.float32)
    ...
    def read_pcloud(self, depth_image_list, cam_intrinsics_list, cam_extrinsics_list):
        ...
        # 返回CPU张量转换为numpy
        return temporal_grid.cpu().numpy()
        ...
        with torch.no_grad():
            while not rospy.is_shutdown():
                try:
                    ...
                    # 执行推理，trav_map形状为 (1, 2, H, W)，1表示batch size，2表示mu和nu通道，H和W为体素网格尺寸100×100
                    ...
                    result_data = {
                        ...
                        'original_img': color_img[0, -1].permute(1, 2, 0).cpu().numpy() * 255,  # 最后一帧作为可视化
                        'ros_timestamp': ros_timestamp,
    ...
    def publishing_loop(self):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                # 添加这段代码进行坐标变换
                # 对每个通道进行转置和旋转处理，使其朝向与RViz中的预期方向一致
                ...
                self.publish_image_messages(corrected_trav_map_np, ros_timestamp)
    ...
    def publish_image_messages(self, trav_map_np, ros_timestamp):
