...
class GlobalMappingNode:
    ...
    订阅局部安全地图与里程计，当车辆停稳≥指定时间后，取最新一帧局部地图，
    将前方一定范围的栅格映射到全局 8000x8000 栅格上，分辨率保持 0.1m，重叠区域取最大值融合。
    """

    def __init__(self):
        rospy.init_node("global_mapping_node")
        ...
        self.global_size = rospy.get_param("~global_size", 6000)  # 与离线版一致，默认 600m 方形
        ...
        self.stop_lin_thresh = rospy.get_param("~stop_lin_thresh", 0.01)
        self.stop_ang_thresh = rospy.get_param("~stop_ang_thresh", 0.01)
        self.stable_time = rospy.get_param("~stable_time", 2.0)
        self.map_lag_guard = rospy.get_param("~map_lag_guard", 1.5)
        self.speed_window_size = rospy.get_param("~speed_window_size", 5)
        ...
        self.save_interval = rospy.get_param("~save_interval", 10.0)  # 秒
        ...
        # 状态机
        self.state = "MOVING"  # MOVING -> WAITING -> (MAPPING once) -> WAITING (latched) -> MOVING
        self.stop_start_time = None
        self.has_mapped = False

        # 缓存
        self.speed_window = deque(maxlen=self.speed_window_size)
        ...
        rospy.loginfo("Global mapping node ready: waiting for odom and local safety maps...")
    ...
    def _is_stationary(self):
        if not self.speed_window:
            return False
        max_lin = max(v for v, _ in self.speed_window)
        max_ang = max(w for _, w in self.speed_window)
        return max_lin < self.stop_lin_thresh and max_ang < self.stop_ang_thresh
    ...
    def odom_callback(self, msg):
        ...
        self.speed_window.append((lin, ang))
    ...
    def control_loop(self, _event):
        ...
        if self.state == "MOVING":
            if stationary:
                ...
                rospy.loginfo("State -> WAITING (vehicle appears stopped)")
        elif self.state == "WAITING":
            if not stationary:
                self.state = "MOVING"
                self.stop_start_time = None
                self.has_mapped = False
                rospy.loginfo("State -> MOVING (vehicle moved)")
            else:
                if self.stop_start_time and (now - self.stop_start_time).to_sec() >= self.stable_time:
                    if not self.has_mapped:
                        success = self.try_mapping()
                        if success:
                            self.has_mapped = True
                            rospy.loginfo("Mapping done. Waiting for next move before re-trigger.")
        else:
            self.state = "MOVING"
    ...
    def try_mapping(self):
        ...
        # 时间检查：两张图都需晚于 stop+lag_guard
        if self.stop_start_time:
            if (s_msg.header.stamp - self.stop_start_time).to_sec() < self.map_lag_guard:
                rospy.logwarn("Skip mapping: safety map is older than stop + lag guard")
                return False
            if (t_msg.header.stamp - self.stop_start_time).to_sec() < self.map_lag_guard:
                rospy.logwarn("Skip mapping: trav map is older than stop + lag guard")
                return False
    ...
    def fuse_local_to_global(self, map_msg, local, pose, yaw_robot, global_arr):
        ...
        cos_o = math.cos(oyaw)
        sin_o = math.sin(oyaw)
        lx = ox + xx * cos_o - yy * sin_o  # 局部坐标系下的栅格中心
    ...
    def start_capture_srv(self, _req):
        ...
        rospy.loginfo("[CAPTURE] start_cap: 全局地图开始采集")
        return EmptyResponse()

    def end_capture_srv(self, _req):
        self.capture_enabled = False
        rospy.loginfo("[CAPTURE] end_cap: 全局地图采集结束")
        return EmptyResponse()
